float waterH(vec3 pos){
	float noise=0;
	noise+=texture2D(noisetex,(pos.xz+vec2(frameTimeCounter)*.5-pos.y*.7)/WATER_CAUSTICS_AMOUNT*1.5).r*1.;
	noise+=texture2D(noisetex,(pos.xz-vec2(frameTimeCounter)*.5-pos.y*.7)/WATER_CAUSTICS_AMOUNT*3.0).r*.8;
	...
	noise-=texture2D(noisetex,(pos.xz+vec2(frameTimeCounter)*.5+pos.y*.7)/WATER_CAUSTICS_AMOUNT*14.0).r*.2;
