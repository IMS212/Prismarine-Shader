...
vec3 GetLightShafts(float pixeldepth0, float pixeldepth1, vec3 color, float dither) {
	...
	#ifdef TAA
		dither = fract(dither + frameTimeCounter * 0.05);
	#endif
	...
	if (isEyeInWater == 1) persistence     *= min(2.0 + rainStrength*rainStrength - sunVisibility * sunVisibility, 8.0) - (cameraPosition.y * LIGHTSHAFT_ALTITUDE_DECREASE_FACTOR * 0.5) * 0.1;
	if (persistence >= 1.25){
	...
	visibility = mix(1.0, visibility, 0.25 + 0.75);
	...
	if (visibility > 0.0) {
		...
		#ifdef OVERWORLD
		vec3 watercol = mix(vec3(1.0), waterColor.rgb * vec3(1, 2 - timeBrightness, 1) / (waterColor.a * waterColor.a), pow(waterAlpha, 0.5));
		#else
		...
		for(int i = 0; i < 16; i++) {
			...
			if (isEyeInWater == 1.0) maxDist = 64;
			...
			if (length(shadowposition.xy * 2.0 - 1.0) < 1.0) {
				...
				else if (isEyeInWater == 1.0) shadow *= watercol * 64 * (1.0 + eBS) * sqrt(minDist / maxDist);
				...
				noise = sin(noise * 32.0 + (frameTimeCounter * 0.25)) * 0.1 + 0.5;
				shadow *= noise;
				#endif
