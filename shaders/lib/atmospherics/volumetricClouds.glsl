...
float getHighQualityVFSample(vec3 pos, float height, float verticalThickness, float samples){
	...
	if (ymult < 4.0){
		noise+= getHorizontalNoise(pos / samples * 1 - wind * 0.5) * 0.5;
		noise+= getHorizontalNoise(pos / samples * 0.5 - wind * 0.4) * 0.75 * VFOG_HORIZONTAL_THICKNESS;
		noise+= getHorizontalNoise(pos / samples * 0.25 - wind * 0.3) * 1.00 * VFOG_HORIZONTAL_THICKNESS;
		noise+= getHorizontalNoise(pos / samples * 0.125 - wind * 0.2) * 1.25 * VFOG_HORIZONTAL_THICKNESS;
		noise+= getHorizontalNoise(pos / samples * 0.076 - wind * 0.1) * 1.50 * VFOG_HORIZONTAL_THICKNESS;
		noise+= getHorizontalNoise(pos / samples * 0.038) * 1.75 * VFOG_HORIZONTAL_THICKNESS;
...
//FINAL
vec2 getHigherVolumetricCloud(float pixeldepth0, float pixeldepth1) {
	vec2 vc 		= vec2(0.0);
	vec4 wpos 		= vec4(0.0);
	...
	for (minDist; minDist < maxDist; ) {
		...
		if (length(wpos.xz) < maxDist){
			...
			float noise = getLowQualityVCSample(wpos.xyz, VCLOUDS_HEIGHT + 10, VCLOUDS_VERTICAL_THICKNESS, VCLOUDS_AMOUNT);
			#elif VCLOUDS_NOISE_QUALITY == 1
			float noise = getHighQualityVCSample(wpos.xyz, VCLOUDS_HEIGHT + 10, VCLOUDS_VERTICAL_THICKNESS, VCLOUDS_AMOUNT);
			#elif VCLOUDS_NOISE_QUALITY == 2
			float noise = getUltraQualityVCSample(wpos.xyz, VCLOUDS_HEIGHT + 10, VCLOUDS_VERTICAL_THICKNESS, VCLOUDS_AMOUNT);
			#endif
			
			float col = pow(smoothstep(VCLOUDS_HEIGHT + 10 - VCLOUDS_VERTICAL_THICKNESS * noise, VCLOUDS_HEIGHT + 10 + VCLOUDS_VERTICAL_THICKNESS * noise, wpos.y), dither);
